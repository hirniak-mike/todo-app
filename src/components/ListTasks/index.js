...
const ListTasks = () => {
  ...
  const { items, isLoadedTodo } = useSelector(({ todoReducer }) => {
    return {
      items: todoReducer.items,
      isLoadedTodo: todoReducer.isLoadedTodo,
    };
  });
  ...
  return (
    <div className={s.listtasks}>
        ...
        <ul>
          {items &&
            items.map((item, index) => (
              <li key={item.id} className={classNames({ [s.active]: item.id === +id })}>
      ...
      <form className={s.listtasks_add}>
        <Input
        ...
        />
        <Button name="Add" onClick={addTitle} isLoading={isLoadingTitle} type="submit" liststyle />
      </form>
    </div>
  );
};
