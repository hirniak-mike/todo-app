...
// refactor
const Tasks = () => {
  const { title } = useSelector(({ todoItems }) => {
    return {
      title: todoItems?.items[0]?.title,
      // tasks: todoItems?.items[0]?.tasks,
    };
  });

  // const tasks = useSelector(({ todoItems }) => todoItems?.items[0]?.tasks);
  ...
  return (
    <div className={s.tasks}>
      ...
      <div className={s.tasks_task}>
        {!isFlag ? (
          <p>No tasks</p>
        ) : (
          <div className={s.tasks_task_wrapper}>
            <Task />
            <Task />
          </div>
        )}
